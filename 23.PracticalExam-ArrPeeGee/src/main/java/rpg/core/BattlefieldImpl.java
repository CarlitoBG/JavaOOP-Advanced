...
public class BattlefieldImpl implements Battlefield {
    ...
    @Override
    public void createSpecial(String heroName, String specialAbilityType) {
        if (!this.participants.containsKey(heroName)) {
            this.writer.writeLine("Participant with that name does not exist.");
        }

        Targetable participant = this.participants.get(heroName);
        List<String> participantInterfacesNames = Arrays.stream(participant.getClass().getSuperclass()
                .getInterfaces()[0].getInterfaces()).map(Class::getSimpleName).collect(Collectors.toList());

        if (participantInterfacesNames.contains("Specialist")) {
            try {
                Specialist specialist = (Specialist) participant;
                Special special = this.specialFactory.create(specialAbilityType);
                specialist.setSpecial(special);
            } catch (Exception e) {
                this.writer.writeLine("Special does not exist.");
            }
        } else {
            this.writer.writeLine("Participant cannot have special abilities.");
        }
    }
