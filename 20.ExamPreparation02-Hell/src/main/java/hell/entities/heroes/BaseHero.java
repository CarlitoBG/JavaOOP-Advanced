...
public abstract class BaseHero implements Hero{
    ...
    BaseHero(String name, int strength, int agility, int intelligence, int hitPoints, int damage, Inventory inventory) {
        this.name = name;
    ...
    public String toString() {
        ...
        try {
            sb.append(String.format("%s: %s", this.getClass().getSimpleName(), this.name)).append(System.lineSeparator())
                    ...
                    .append(String.format("###Items: %s", this.getItems().size() == 0 ? "None" :
                            this.getItems().stream().map(Item::getName).collect(Collectors.joining(", "))))
